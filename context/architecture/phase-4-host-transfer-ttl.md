...
## Status
Implemented e2e with socket + REST tests. Backend 39 pass
(db 5 + api 6 + stock 8 + cart 10 + realtime 1 + checkout 4 + lifecycle 4 + ttl-touch 1);
...
| `backend/expiry.js` | `isSessionExpired`, `expireSessionIfNeeded(db, session)` (opportunistic expiry on any REST/socket touch), `touchSession(db, session)` (slides `expires_at = now + 30 min` on joins / cart mutations / ready toggles; never resurrects dead sessions; reads never touch), `SESSION_TTL_SECONDS` (env-overridable, default 1800), `dropEphemeralState` (participants + cart + reservations), `sweepExpiredSessions({io})` (marks past-`expires_at` `active` rows `expired`, clears memory, broadcasts `session:expired` + `error SESSION_EXPIRED` per room). |
...
- Expiry is now true sliding-idle (`expires_at = last_activity + 30 min` via `touchSession`, FR-13) — replaced the old fixed `created_at + 24h` deadline; see `session-ttl-touch.md`.
